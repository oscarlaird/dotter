...
async def process_input_texts(input_texts, prefix_cache_trie=None, batch_texts_are_siblings=False, cached_results=None, tokenizer=tokenizer, model=model):
    ...
    results = []
    last_token_lengths = [len(tokenizer.decode([int(input_ids[i][-1])])) for i in range(batch_size)]
    for i in range(batch_size):
        ...
        #
        result = (np.log(clean_last_token_probs), clean_last_token_probs_cumulative, np.log(stop_token_prob), last_token_lengths[i])
    ...
    while True:
        ...
        if queue:
            ...
            for i, result in enumerate(results):
                probs, cum, stop_prob, last_token_length = result
                print(cum.dtype, probs.dtype)
                ...
                response = {'type': 'processed', 'ftp': input_texts[i], 'probs': probs, 'cum': cum, 'stop_prob': stop_prob, 'last_token_length': last_token_length}
                # todo: time this
                try:
...
print(get_prefix_range('em', clean_tokens))
print(get_prefix_range('en', clean_tokens))
print(get_prefix_range('eo', clean_tokens))
clean_tokens[11578], clean_tokens[11579]
