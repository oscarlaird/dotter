...
# #%% evaluate the model
# phrases_file = "../frontend/static/phrases.txt"
...
import json
with open(_BACKEND_DIR / 'prefix_range_precomp.json', 'w') as f:
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    while True:
        ...
        if message['type'] == 'reset':
            ...
            try:
                with open(_BACKEND_DIR / 'log.txt', 'r') as f:
                    lines = []
                    for line in f:
                        x = json.loads(line)
            ...
            with open(_BACKEND_DIR / 'log.txt', 'a') as f:
