<script>
    ...
    // trie_store.subscribe(new_trie => {
    //     trie = structuredClone(new_trie);
    ...
    function timer_likelihood(time, phase, likelihood_model) {
        ...
        return logaddexp(not_outlier_prob + gaussian_log_likelihood, outlier_prob + uniform_log_likelihood);
    }
    ...
    function click(event) {
        ...
        let new_likelihoods = {};
        // let pDATA = -Infinity;
        // let total_logprob_timer_targetted = -Infinity;
        visible_nodes.forEach(node => {
            ...
            // if (children_cond_post > 1e-1) {
            //     console.log("WARNING: children_cond_post is not 0.0 for node " + node.val + " with children_cond_post " + children_cond_post);
            //     if (children_cond_post > 0) {
            //         children_cond_post = 0;
            //     }
            // }
            // let logprob_timer_targetted = node.post_Z + Math.log(1 - Math.exp(children_cond_post));
            // if (isNaN(logprob_timer_targetted)) {
            //     throw new Error("logprob_timer_targetted is NaN for node " + node.val);
            // }
            // total_logprob_timer_targetted = logaddexp(total_logprob_timer_targetted, logprob_timer_targetted);
            // pDATA = logaddexp(pDATA, logprob_timer_targetted + node_timer_likelihood);
        });
        // check that total_logprob_timer_targetted is approximately 0
        // for (let node of visible_nodes) {
        //     new_likelihoods[node.val] -= pDATA;
        // }
        // we want log(P(D|x)/P(D)) = log(P(D|x)) - log(P(D))
        // console.log("pDATA delta estimated by the visualizer", pDATA);
        keep_phases = false;
    ...
    function setLocations(node, root_call = false,  loc = null, size_height = FIRST_BOX_HEIGHT, size_width = BOX_WIDTH) {
        ...
        let keep_nodes_when_keep_phases = true;
        let visible_children = node.children.filter(child => child.is_visible || (keep_nodes_when_keep_phases && child.val in old_visible_registry && keep_phases));
        ...
        if (visible_children.length > 0) {
            ...
            // if (Math.abs(total_children_post - node.post) > 1e-1) {
            //     console.log("WARNING: children_cond_post is not 0.0 for node " + node.val + " with post " + node.post + " and total_children_post " + total_children_post);
            // }
    ...
    function draw() {
        ...
        const visibleNodes = visible_nodes;
        ...
        visibleNodes.forEach(node => {
            ...
            if (node.val.length >= 1) {
                ...
                const endX = get(parent_node.tweened_location).x + get(parent_node.tweened_size_width) - get(left_offset) - TIMER_CIRCLE_RADIUS;
...
<div class="flex flex-row">
    <input type="checkbox" bind:checked={developer_visualizer}/>
    <label>Developer Mode</label>
    <input type="checkbox" bind:checked={show_boxes}/>
